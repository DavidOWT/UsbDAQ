...
mySampleRate = str2double(DAQInput{2});     %   Session frequency
...
myNumPretriggerBuffers = 1;                 %   How many refill periods before trigger?
myNumChannels = int8(sum(ActChn));          %   Number of active chane;s
...
%   Make log-file
[fid1, DAQInput] = CreateLog(DAQInput, ActiveChannels);
...
if not(optionCont == 1)
    ...
    RecData = ExtractData(DAQInput{3});
...
%   Trigger
trig = event.Data(:,Trig.Chan) > Trig.Val; % triggers when digital signal is high
...
if dataBeingLogged == true
    if myCurrentBuffer == (captureLength/myBufferRefillPeriod) + 1 % +1 to account for the padded buffer (line 27)
        ...
        RecordData(reqCapture, fid1, Frmt) 
...
function [fid1, DAQInput] = CreateLog(DAQInput, ActiveChannels)
%   Function to setup the logging output text file
%
TimeStamp = clock;
DAQInput{3} = sprintf('%s_Date%0.2i%0.2i%0.2i_Time%0.2i%0.2i%0.2i%s', DAQInput{3}(1:end-4), TimeStamp(1:5) , ...
                round(TimeStamp(end)), DAQInput{3}(end-3:end));
fid1 = fopen(DAQInput{3},'w');

% Write log-file header
WriteHeader(fid1, DAQInput, TimeStamp, ActiveChannels)

end


function WriteHeader(fid, DAQInput, TimeStamp, ActiveChannels)
%   Function to write a header for the logging text file
%
% Write header to data log file
A{1} = '---------- DAQ NI USB-6003 Output File ----------';
A{2} = sprintf('%s - Filename', DAQInput{3});
...
if event.TimeStamps(1) == 0
    if exist('TrigVal', 'var')
        PltTrig = plot([0 10], [TrigVal TrigVal]);
